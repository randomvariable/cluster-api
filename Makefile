...
test: $(GINKGO) ## Run tests. Specify number of nodes with GINKGO_NODES= (default 1)
	source ./scripts/fetch_ext_bins.sh; fetch_tools; setup_envs; GODEBUG=asyncpreemptoff=1 $(GINKGO) -skipPackage e2e,infrastructure/docker -failFast -progress -race -stream -progress -nodes $(GINKGO_NODES) test -v ./... $(TEST_ARGS) -v10
