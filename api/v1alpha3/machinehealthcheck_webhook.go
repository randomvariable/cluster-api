...
var (
	...
	disableMinimumNodeStartupTimeoutForTest = false
)
...
func (m *MachineHealthCheck) validate(old *MachineHealthCheck) error {
	...
	if m.Spec.NodeStartupTimeout != nil && m.Spec.NodeStartupTimeout.Seconds() < 30 && !disableMinimumNodeStartupTimeoutForTest {
		allErrs = append(
			allErrs,
...
// DisableMinimumNodeStartupTimeoutForTest disables the minimum machinehealthcheck StartupTimeout value, so it does not need to be at least 30s
func DisableMinimumNodeStartupTimeoutForTest() {
	disableMinimumNodeStartupTimeoutForTest = true
}
