...
| KCPReconcile | EvaluateRemediationAdmission | controlplane/kubeadm/internal/controllers/remediation.go:595 (`canSafelyRemediateMachine`) | First call site: remediation reconcile snapshots the current safety predicate and records whether the Machine is admitted to proceed toward the pre-terminate hook. |
| KCPReconcile | PreTerminateHookGateRecheck | controlplane/kubeadm/internal/controllers/controller.go:1433-1460 (pre-terminate hook entry), `controlplane/kubeadm/internal/controllers/remediation.go:595` | Second call site: pre-terminate hook re-reads the fresh safety predicate. In the buggy variant it still proceeds based on the earlier admission snapshot. |
| KCPReconcile | PreTerminateHookGateRecheckStrict | same | Fixed variant: pre-terminate hook re-evaluates the fresh safety predicate and blocks/reverts instead of proceeding on a stale admission snapshot. |
| KCPReconcile | ExternalMembershipChangeDrop | companion to `controlplane/kubeadm/internal/controllers/remediation.go` and sibling remediation / external etcd changes | Abstract a sibling remediation or external etcd membership change landing between admission and hook-time recheck. |
