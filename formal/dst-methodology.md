...
| `Partition(m)` (network-only variant) | `docker network disconnect kind <node-container>` | Detaches the node from the kind docker network entirely. Cleaner than `docker pause` when the test wants kubelet/etcd processes to keep running locally but lose all network reachability — exactly the FM-3 partition shape. | `docker network connect kind <node-container>`. Used by `test/e2e/fm3_etcd_unreachable.go`. |
| `LbBroken` (used by FM-13) | `docker pause <cluster-name>-lb` | See entry above; called out separately because `test/e2e/fm13_apiserver_lb_broken.go` is the canonical reproducer. | `docker unpause`. |
| `ApiServerRestart(m)` (FM-19 transient) | `docker exec <node-container> sh -c 'crictl ps -q --label io.kubernetes.container.name=kube-apiserver | xargs -r crictl rm -f'` | Kills the apiserver static pod inside the CP node; kubelet relaunches it within seconds. The relist storm covered by FM-19 plays out during the restart window. | n/a — kubelet restarts the pod. Used by `test/e2e/fm19_apiserver_restart.go`. |
| `DrainBlocked(m)` (FM-23) | `kubectl apply` of a `policy/v1` PDB with `maxUnavailable: 0` against a sticky pod tolerating the CP `NoSchedule` taint and pinned (`spec.nodeName`) to the target CP Node. Then add `mhc-test=fail` to the host Machine to trigger remediation. | KCP sets `deletionTimestamp` on the Machine but the eviction stalls because the PDB refuses every drain attempt. | Delete the PDB; eviction succeeds and KCP completes the deletion. Used by `test/e2e/fm23_drain_blocked_pdb.go`. |
| `RollingUpgrade(c)` (FM-15) | `clusterctl`-driven version bump via `framework.UpgradeControlPlaneAndWaitForUpgrade`. No fault injection. | Each Machine is replaced one at a time; the cluster never drops below quorum. | n/a — happy-path liveness reproducer. Used by `test/e2e/fm15_upgrade_in_flight.go`. |
