...
| `KCPReconcile.tryGetEtcdMemberName` | [`KubeadmControlPlaneReconciler.tryGetEtcdMemberName`](../controlplane/kubeadm/internal/controllers/remediation.go#L640) | The matchable-set lookup. Returns empty string when no Machine matches the etcd member by node name — exactly the user-incident state. |
...
| `EtcdMembership.ForwardLeadership` (transient pre-RemoveMember) | [`Workload.ForwardEtcdLeadership`](../controlplane/kubeadm/internal/workload_cluster_etcd.go#L96) | Currently NOT explicitly modelled in `Lifecycle.qnt`. Issue corpus #IC-09 covers the gap. |
