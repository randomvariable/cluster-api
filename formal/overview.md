...
| OidcIssuerRotation.qnt | 2 | 300×40 | **2/2 stable issuer/key bookkeeping invariants** under random walk; explicit stale-issuer/key counterexample `NoExtendedAuthOutage` reachable at Apalache depth 4 | — |
...
| KCPReconcile.qnt (condemned peers) | 3 scenarios + depth-4 check | 3-node second admission blocked, 5-node second admission allowed, all-deleting carve-out admitted; strengthened invariant `NoJointQuorumLossAcrossConcurrentRemediation` passes at Apalache depth 4 | — |
